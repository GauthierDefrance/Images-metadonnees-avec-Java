...
public class Console {
    ...
    private String executeDirectoryMode() throws ImageProcessingException, IOException {
        ...
        if (snapshotsave) {
            ...
            folder.snapshotSave(this.cmd.getOptionValue("ss"));
        }
        if (snapshotcompare) {
            output.append("Comparaison des snapshots activée.\n");
            System.out.println("Resultat :"+folder.snapshotCompare(this.cmd.getOptionValue("sc")));
            folder.snapshotCompare2(this.cmd.getOptionValue("sc"));
        }
